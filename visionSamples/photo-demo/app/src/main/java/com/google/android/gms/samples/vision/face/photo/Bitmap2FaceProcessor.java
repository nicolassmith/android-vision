...
public class Bitmap2FaceProcessor {
    private static final String TAG = "Bitmap2FaceProcessor";

    static SparseArray<Face> getFaceSparseArray(Context applicationContext, Bitmap bitmap) {
        ...
        if (!safeDetector.isOperational()) {
            ...
            boolean hasLowStorage = applicationContext.registerReceiver(null, lowstorageFilter) != null;

            if (hasLowStorage) {
                Toast.makeText(applicationContext, R.string.low_storage_error, Toast.LENGTH_LONG).show();
                Log.w(TAG, applicationContext.getString(R.string.low_storage_error));
            }
        }
